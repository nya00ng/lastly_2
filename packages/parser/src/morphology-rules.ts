...
const ACTION_STEM =
  '(?:빨|갈|씻|닦|주|줘|줬|먹|넣|열|닫|잠그|교체|바꾸|정리|청소|설치|세탁|치우|버리|비우|충전|소독|살균|점검|복용|물갈이|분갈이|빗질|양치|세차|환기|산책|제거|널|깎|개|채우|삶|뒤집|돌리|말리|털|접|헹|묶|옮|볶|다듬|달|꿰매|불리|깔|쌓|치대|부치|뿌리|맞추|걸)';
...
export function readMorphologySignals(text: string): MorphologySignals {
  ...
  const intended =
    ...
    /(?:할|갈|빨|먹|갈|닦|청소|정리)\s*(?:거야|게|래)/.test(normalized) ||
  ...
  return {
    ...
    nonAssertion,
    nonActionPredicate,
    completed: PAST_ENDING.test(direct),
  };
}
